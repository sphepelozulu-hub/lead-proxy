  ...
  function postToLeadbyte(postData, res) {
    ...
    const r = https.request(options, resp => {
      ...
      resp.on('end', () => {
        ...
        try {
          res.end(JSON.stringify(JSON.parse(rb)));
        } catch (e) {
    ...
    r.on('error', e => {
      console.error('LeadByte request error:', e.message);
  ...
  if (req.url === '/submit-cartrack' && req.method === 'POST') {
    readBody(req, body => {
      try {
        ...
        // IMPORTANT:
        // These are the actual CarTrack LeadByte field names.
        p.append('First_Name', firstName);
        p.append('Last_Name', lastName);
        p.append('CellNumber', cellNumber);
        ...
        p.append(
          'optinurl',
          incoming.optinurl || 'http://url.com'
        );

        // Opt-in date
        p.append(
          'optindate',
          incoming.optindate || getOptinDate()
        );
        ...
        // CarTrack Cameras Offer
        p.append('offer_id', '3046');

        console.log(
          'CarTrack submission fields:',
          {
  ...
  if (req.url === '/submit-loans' && req.method === 'POST') {
    readBody(req, body => {
      try {
        ...
        p.append(
          'idnumber',
          d.params?.idnumber || generateSAID()
        );
